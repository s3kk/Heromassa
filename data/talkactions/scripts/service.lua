local config = {
    priceForEachMessage = 1000000, -- preço de cada mensagem
    levelToUseQuestService = 100 -- level para poder usar este comando
}
...
function onSay(cid, words, param, channel)
    ...
    if(doPlayerRemoveMoney(cid, config.priceForEachMessage) == TRUE) then
        doBroadcastMessage("Service: " .. getPlayerName(cid) .. "'s now starting a " .. t[1] .. "! Preço: " .. t[2] .. "k.", MESSAGE_STATUS_WARNING)
    else
